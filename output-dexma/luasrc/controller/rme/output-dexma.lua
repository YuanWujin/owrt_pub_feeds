...
function action_diag()
    local xxx, code = pl.utils.execute("pidof output-dexma 2>&1 >/dev/null")
    io.stderr:write(string.format("pidof retrned <%s> <%s>\n", tostring(xxx), tostring(code)))
    ...
    local rval = {
        ...
        custom_diags = "/diags2", -- TODO - link to the sub page? true,  -- means a link to the account page will always be shown
        custom_good = summary,
    }
