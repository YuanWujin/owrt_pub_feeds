...
-- FIXME - make sure we create three phase points correctly?
local function get_did_from_model(model, devid, dtype, channel)
	for mdevid, mdev in pairs(model) do
		if mdevid == devid then
			...
			if not channel and type(mdev.branches) == "table" and #mdev.branches == 1 then
				return string.format("%s-%s", mdev.cabinet, mdev.branches[1].label)
			end
			if not channel then return nil, "Channel not specified and but didn't match a mains type device?" end
			for _, mbranch in pairs(mdev) do
				...
				-- FIXME - look at how the model works on mains please!
			end
		end
	end
end
...
local function handle_message_data(mid, topic, jpayload, qos, retain)
	...
	local did = get_did_from_model(cabinet_model, device, dtype, channel)
	if did then
	...
	-- FIXME - this method doesn't do a very good job of including multiple points from a given point.
	-- ie, we might want to send max and average current under the same 'did'!
	-- fuck it....(does it matter? does dexma actually care if the same did is posted twice? try it and see?
...
local function httppost(url, data, userheaders, opts)
	...
	if r then
		print("Successful 'request', dump headers: " .. pl.pretty.write(h))
...
local function flush_qd()
	...
	for ts, data in pairs(state.qd) do
		...
		local httpok, c, h, body = httppost(url, data.values, headers, {verify={}}) -- FIXME - temporary for testing locally!
		if httpok then
